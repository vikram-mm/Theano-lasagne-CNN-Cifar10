...
def train1_net(network, datasets, log_path):
    ...
    log(f,'7 Layer, HeNorm, With regularization')
    ...
    for epoch in range(num_epochs):
        ...
        for i, (X, Y) in enumerate(datasets['train']):
            ...
            loss, accuracy = network.train_bot_up_net(X, Y, LR)
            train_loss += loss
        ...
        p1, = plt.plot(100*td_losses[0, : epoch],label='Training accuracy')
        p2, = plt.plot(100*td_losses[1, : epoch],label='Validation accuracy')
        plt.legend()
	plt.ylabel('ACCURACY %')
        plt.xlabel('EPOCH NUMBER')
    ...
    _, _, txt = test_net(network, datasets['test'], 0)

...
from lasagne.layers import Conv2DLayer as ConvLayer
############################################################
class gradControlClass(UnaryScalarOp):
  ...
  def c_code(self, node, name, inp, out, sub):
        x, = inp
        z, = out
	return "%(z)s = (%(x)s<1&&%(x)s>0) ? 1.0f : 0.0f);" % locals()
gradControl  = gradControlClass(same_out_nocomplex, name='gradControl')
############################################################
class signClass(UnaryScalarOp):
    # @staticmethod
    # def static_impl(x):
    #     return x/abs(x)
    #
    # def impl(self, x):
    #     return signClass.static_impl(x)
    def grad(self, inp, grads):
        x,=inp
	gz, = grads
	print(type(x))
    ...
    def c_code(self, node, name, inp, out, sub):
        ...
        z, = out
	#print('x : %r' % (x))
	#print("%(z)s = ((%(x)s >= 0.0f) ? 1.0f : -1.0f);" % locals())
	#exit(0)
        return "%(z)s = ((%(x)s > 0.0f) ? 1.0f : 0.0f);" % locals()
scalar_sign  = signClass(same_out_nocomplex, name='scalar_sign')
sign = Elemwise(scalar_sign, name='sign')
